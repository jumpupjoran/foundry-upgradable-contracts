...
NETWORK_ARGS_SEPOLIA := --rpc-url $(SEPOLIA_RPC_URL) --private-key $(PRIVATE_KEY) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) 
...
deploy-sepolia:;@forge script script/DeployBox.s.sol:DeployBox $(NETWORK_ARGS_SEPOLIA)


upgrade:
	@forge script script/UpgradeBox.s.sol:UpgradeBox $(NETWORK_ARGS)

upgrade-sepolia:;@forge script script/UpgradeBox.s.sol:UpgradeBox $(NETWORK_ARGS_SEPOLIA)
